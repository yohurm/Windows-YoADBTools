...
├── states.css      # .yovo-interactive 选中 ripple + .yovo-focus-ring / --host
└── styles.css      # @import theme + states（@yovo/ui/theme.css 导出指向此文件）
```

**公开组件一律 `Yo*` 标注**（禁止 `YButton` 这类单字母前缀）；CSS/token 命名空间保持 `yovo-*`。
...
| 数据 | **YoVirtualList**（日志行 + 文件四列清单 + `.yovo-interactive` 选中片）、YoTree（终端命令库） |
...
| **S4 日志模块（2–3 周）** | yovo-logsrv + YoVirtualList + 多会话 Tab + AS 过滤栏 + 导出 | 性能验收全绿（§12 末行） |
...
| 阶段 | 状态 | 落地要点与验证 |
|------|------|----------------|
| **S1 骨架** | ✅ | Cargo workspace 五 crate；Tauri 壳 25 条命令全量接线；@yovo/ui 组件库（token 单源，深浅主题）；@yovo/api 契约层（fixture 单测）；工作台壳（设备栏/导航/设置/状态栏/模块注册表）；sidecar adb |
