...
| `SessionLogService` | 逐窗口实时日志文件：`open/append/close/list/latest`；写到 `ModuleData/log-analyzer/session-logs/`，**按 seq 去重、不受 RingBuffer 容量约束**（记录完整）；写入方式 `LogWriteMode`（覆盖=固定文件截断 / 续写=每次任务新开文件）；`export` 合并选定源文件到一份 txt |
...
| 复合 | YoTabs、YoDialog、YoToast / YoToaster、YoEmptyState、YoLoading、YoContextMenu / **YoContextMenuHost** |
| 图标 | Icon、YoFileIcon |
...
5. 日志写入（ADR-v6-006 消费端过滤）：core `SessionLogService` 只把 **UI 已过滤的行**写入逐窗口文件（不受 RingBuffer 容量约束）；`log.export` 合并选定源文件，目标路径为空则取 `export_default_path`，都空由 core 生成时间戳文件。commands 只转发。
