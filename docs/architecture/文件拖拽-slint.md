...
不新开 crate。不引入 `drag` 类插件（载荷是本机 `CF_HDROP`，解决不了设备文件）。
...
窗口平台拖放保持默认启用；关闭后 Explorer 拖入将拿不到完整路径。
...
```text
Explorer CF_HDROP
  → 窗口平台 IDropTarget（子 HWND；AllowExternalDrop=false）
  → drop-event { paths, position }
  → FileView：命中测试
...
| `app/yohu-app/src/dnd/` | **新建**；`lib.rs` 组装 |
...
| 期 | 用户路径 | 代码 |
|----|----------|------|
| **P1 拖入** | 从 Explorer 把文件或文件夹丢进当前目录 / 目录行 | 平台 drop 事件 + 命中 + `TransferRunner` 目录 + overlay |
