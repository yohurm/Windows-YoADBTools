...
- **显示面板：** 窗口私有、append-only。只在本窗口重新开始采集（新流）或用户清空 / 清设备缓冲时 flush。设备无输出、掉线、停采、进程重绑、跟滚、镜像落后都不得冲掉已画出的行。用户改级别/Tag/关键字时从已有行筛选，再按 seq 从镜像补新命中。镜像只补洞，禁止整表替换。同窗口 adopt 续采保留 `fromSeq` / 面板。新建/复制仍空且停；**点开始**才订阅：先 `log.processSnapshot` 绑 PID，`fromSeq=0`，按本窗口过滤从当前环补齐，再跟新行。禁止新窗口从尾 seq 起空等下一次 logcat。WebView 恢复可见性时 replay 补镜像。
