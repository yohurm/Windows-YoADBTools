...
| # | 功能 | 说明 |
|---|------|------|
| F20 | **软换行开关** | 对齐 AS Soft-Wrap；默认开 |
| F21 | **自动滚底** | 贴底跟随；用户上滚解锁（BeautyCat） |
| F22 | **清空设备缓冲按钮** | 与设置项独立的即时操作 |
| F23 | **复制选中行 / 复制可见** | 剪贴板 |
| F24 | **关键字命中高亮** | 行内高亮，非正则 |
| F25 | **PID 过滤** | 文本框，精确或包含 |
| F26 | **崩溃/异常粗信号** | 简单模式：消息含 `FATAL EXCEPTION` / `AndroidRuntime` / `ANR in` 时行强调 + 计数 |
| F27 | 过滤变更后**重放缓冲到可见列表** | 改级别/Tag/关键字立即重刷 VisibleLines |
| F28 | 世代/启动失败 UX | 失败提示明确；确保 Drain 永不卡死（回归测试） |
...
| 阶段 | 内容 | 预估 |
|------|------|------|
| **已完成基线** | F01–F12；去正则改关键字；设置三项 | — |
| **迭代 A** | F20–F24、F27、F28 回归加固 | 约 1 周 |
| **迭代 B** | F25–F26 信号 + PID | 约 0.5–1 周 |
| **迭代 C** | F30–F35 按需挑选 | 另立 |
