...
- 布局：工具栏（上传 / 刷新 / 预览开关）→ 路径栏（面包屑从安全根起：`sdcard ▸ DCIM`，逐级可点 + 上级停在根上）→ 资源管理器主区（四列清单 + 可收起预览）→ 传输面板。
- **四列清单**：名称（默认 240px，过长省略 + title 全名）/ 类型 / 大小 / 日期（末列 `1fr` 吃剩余）。表头短标签（名称/类型/大小/日期），预留排序箭头槽，列间短分隔线；前三列 `YColResizer` 可拖。类型列用短标签（目录/链接/扩展名）；日期显示 `MM-DD HH:mm`。目录与文件同一列表，目录优先；双击目录下钻。无独立「目录栏」。
