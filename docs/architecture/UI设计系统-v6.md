...
- 行结构（列对齐，等宽）：`[时间 18] [PID 6→] [级别 1] [Tag ≤24] [消息 →]`；级别用色字 + 细左条（3px）；Fatal 反色块。
- 行交互：hover 整行 `--yovo-accent-soft` 4% 底；点击选中（后续复制/书签）；信号行（崩溃/ANR）行底色 `--yovo-signal-bg` + 左侧 Error 条。
- 过滤栏：级别/包名/进程/PID/检索 五项，控件统一 26px 高；检索框带放大镜图标与「清除」；过滤生效时检索框加 accent 边框提示。
- 会话 Tab：标题 + 采集绿点/信号红点 + 关闭 × + 新建 +；右键菜单（关闭其他/重命名/复制会话）；Tab 溢出可横向滚动。
- 状态行：`采集指示 · 设备 · 缓冲 n · 可见 n · 信号 n · 进程索引 n s 前 · 滞后回补提示`。
- 空态：未采集 → 插画图标 + 「点击开始采集」主按钮；过滤无命中 → 「无匹配日志，调整过滤条件」。
...
- 布局：工具栏（上传/新建目录/刷新）→ 路径栏（面包屑化：`sdcard ▸ DCIM ▸ Camera`，逐级可点）→ 双栏（目录列表 | 文件列表）。
- 目录列表：仅目录（树形，含折叠）；文件列表：当前目录文件（名称/大小/修改时间，等宽右对齐）。
- 文件图标：按扩展名分类色（媒体/文档/APK/其他）。
- 传输面板：卡片式（方向图标 + 文件名 + 进度条 + 速度 + 取消）；完成 3s 后淡出。
