...
5. **投屏显示** — 官方 `scrcpy-server` 4.1 sidecar + `yohu-mirror` 自写客户端（USB reverse 优先，`tcp:` 默认 forward）；投屏协议 usb/wifi；帧在壳内 Media Foundation → D3D11 YUV HWND（对齐 YoPanel，ADR-v6-024）；默认可操作，页眉可切仅显示；每设备一路 + generation
