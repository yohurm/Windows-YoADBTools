...
- 架构：`docs/architecture/README.md`（分层/IPC/模块/ADR-v6-001～025）；右键菜单见 `docs/architecture/右键菜单-v6.md`
...
5. **投屏显示** — 官方 `scrcpy-server` 4.1 sidecar + `yohu-mirror` 自写客户端（USB reverse 优先，`tcp:` 默认 forward）；投屏协议 usb/wifi；帧在壳内 Media Foundation → D3D11 YUV HWND（对齐 YoPanel，ADR-v6-024）；默认可操作，页眉可切仅显示；设备深浅色读统一状态 Hub，不是工作台 theme；每设备一路 + generation
