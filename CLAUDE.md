...
5. **投屏显示** — 官方 `scrcpy-server` 4.1 sidecar + `yohu-mirror` 自写客户端（USB reverse 优先，`tcp:` 默认 forward）；投屏协议 usb/wifi；帧在壳内 Media Foundation → D3D11 YUV HWND（ADR-v6-024）；**舞台像素由 HWND 独占**（空态/加载/暂停也在同一表面，ADR-v6-026）；**UI 只报 avail，HWND 按画面 contain 并画占用卡片**（ADR-v6-027；WebView 舞台是透明洞，禁止运行时 `containInZone` / CSS 占用过渡）；默认可操作，页眉可切仅显示；设备深浅色读统一状态 Hub，不是工作台 theme；每设备一路 + generation
