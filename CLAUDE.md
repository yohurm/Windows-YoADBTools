...
- **S1 骨架未开始**：Cargo workspace + Tauri 壳 + @yovo/ui 基础组件 + sidecar adb + 设置/设备栏（验收：空壳安装包 ≤ 12 MB）
- 当前仓库中的 `src/`（C#/WPF）与 `YovoAdbTools.sln` 为 **v5 遗留**：仅存档参考，不作为 v6 实现依据；S5 前旧版继续可用，S5 随新架构全功能验收后下线
