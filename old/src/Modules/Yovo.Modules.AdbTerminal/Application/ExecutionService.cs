...
/// 平台能力只依赖 IAdbCommandExecutor 切片（ISP）。
/// </summary>
public class ExecutionService
{
    ...
    public ExecutionService(IAdbCommandExecutor adb, IAppLog log)
    {
        _adb = adb;
        _log = log;
    }
    ...
    public async Task<CommandResult> ExecuteAsync(
        DeviceSerial serial, CommandDefinition cmd, string[]? inputs, CancellationToken ct = default)
    {
        // ===== 输入校验（结构化失败，不抛异常） =====
        ...
        if (ContainsUnresolvedPlaceholder(resolved))
            _log.Warn($"命令 \"{cmd.Name}\" 含未替换占位符: {resolved}", AdbTerminalModule.ModuleId);
    ...
    public async Task<GroupResult> ExecuteGroupAsync(
        DeviceSerial serial, CommandGroup group, string[]? inputs,
    ...
    private async Task<CommandResult> RunWithResultAsync(
        CommandDefinition cmd, DeviceSerial serial, string resolved, CancellationToken ct)
    {
        var result = new CommandResult
        {
            Command = $"adb {resolved}",
            CommandName = cmd.Name,
            DeviceSerial = serial.Value,
            Timestamp = DateTime.Now
        };

        try
        {
            var raw = await _adb.ExecuteAsync(serial, resolved,
                TimeSpan.FromMilliseconds(cmd.TimeoutMs), ct);
    ...
    private static CommandResult Fail(CommandDefinition cmd, DeviceSerial serial, ResultSource source, string error)
        => new()
        {
